...
int telegram_alert_point = 65;
int telegram_delay_time = 3600000;
long interval = 60000;
int sample_time = 200;
...
String documentPath = "no8readings/watertank/readings";
...
int cnt = 0;
int first_round = 1;
int telegram_run_flag = 1;
int telegram_cnt = 0;
unsigned long previousMillis = 0;
unsigned long telegramBufferMillis = 0;
...
{   
  ...
  if (app.ready() && currentMillis - previousMillis >= delay_time)
  {
      ...
      //TELEGRAM CODE//      
      if(telegram_run_flag == 1 && got_percentage < telegram_alert_point){
        if (WiFi.status() == WL_CONNECTED) {
        //bot.sendMessage(CHAT_ID, "ALERT", "");
        HTTPClient http;
        Serial.println(telegramGroup + String(got_percentage));
        http.begin(telegramGroup + "🚨ALERT🚨: Water Level is at " + String(got_percentage) + "%");
        int httpResponseCode = http.GET();
        Serial.print("HTTP Response code: ");
        Serial.println(httpResponseCode);
        http.end();    
        telegram_cnt = telegram_cnt + 1;
        } 
        else {
      ...
      if (telegram_cnt >= 2){
        Serial.println("BUFFER RATE EXCEEDED ");        
        telegram_run_flag = 0;
        telegram_cnt = 0;
      }

      if (currentMillis - telegramBufferMillis >= telegram_delay_time){
        Serial.println("1 hour has passed");
        Serial.println("Current Telegram Timer: " + String(telegramBufferMillis));
        telegramBufferMillis = currentMillis;
        telegram_run_flag = 1;
      }
      //TELEGRAM CODE//
  }
